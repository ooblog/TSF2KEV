...
function! KEV2pushmenu()
    ...
    :for s:inputkey in range(s:KEV2_keyslen)
        ...
        execute "amenu  <silent> " . (s:KEV2_menuid+0) . ".01 「鍵盤." . s:KEV2_kbdnamemenu . ( s:KEV2_choicekanaidx%s:KEV2_keyslen == s:inputkey ? "✓" : "" ) . " <Plug>(KEV2imap_" . s:commandkana . ")"
    :endfor
    let s:KEV2_choicedicmenuname = escape(s:KEV2_choicekana . "[" . s:KEV2_dickana . "]",s:KEV2_menuESCs)
"    let s:KEV2_choicedicmenuname = ( count(["\(ろ)","\(ロ)"],s:KEV2_choicedicmenuname) ? "\\" . s:KEV2_choicedicmenuname : s:KEV2_choicedicmenunam )
    execute "amenu  <silent> " . (s:KEV2_menuid+1) . ".01 " . s:KEV2_choicedicmenuname . ".test <Plug>(KEV2help)"
endfunction
