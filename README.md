...
☑「#!TSF_calc」内の「D」演算子で「1|3D&#126;10」のように任意桁で分数の小数変換ができるように。式の中で小数の桁数指定をするのでTSF1KEVにあった浮動小数の丸め指示「#TSF_calcRO」も圧縮予定。  
☑小数の制度が「#!TSF_calc」内の「1|3D&#126;10」式内で処理できたからの、pythonでdecimalモジュールの代わりに多倍長整数が直接使えたので様子見(Calcの軽量化)。  
