...
☑「#!TSF_RPN」で指数表示になるような桁数でもアラビア数値に修正。ただし浮動小数の計算なので端数が丸められる。  
☑「#!TSF_calc」内の「kM&#126;1&#126;10」で1から10まで合計する和数列(総和)、「kP&#126;1&#126;10」で1から10まで階乗する積数列(総乗)を用いて乗数や階乗の計算。  
☐「#!TSF_calc」内の「D&#126;10」を用いて通常は浮動小数丸め処理される分数の小数変換を精度を上げる事ができるようにする予定。式の中で小数の桁数指定をするのでTSF1KEVにあった浮動小数の丸め指示「#TSF_calcRO」も圧縮予定。  
☐小数の制度が「#!TSF_calc」内の「D&#126;10」式内で処理できるのなら、pythonでdecimalモジュールの代わりに多倍長整数が直接使えそう(Calcの軽量化)な予感。  
