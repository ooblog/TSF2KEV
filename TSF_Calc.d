...
string TSF_Calc_bracketsJA(string TSF_calcQ){    //#TSFdoc:分数電卓の日本語処理。(TSFAPI)
    ...
    if( TSF_calcA.front!='n' ){
        ...
        if( count(TSF_calcA,"|") ){
            ...
            TSF_calcNstr=TSF_calc_comma_okusen(TSF_calcNstr,TSF_Calc_okusenyen,4,true).stripLeft('0').replace("円","");
            if( TSF_calcNstr.length==0 ){ TSF_calcNstr="0"; }
            TSF_calcDstr=TSF_calc_comma_okusen(TSF_calcDstr,TSF_Calc_okusenyen,4,true).stripLeft('0').replace("円","");
            TSF_calcA=join([TSF_calcDstr,TSF_calcNstr],"分の");
            if( count(TSF_calcA,"1分の") ){
                TSF_calcA=join([TSF_calcA.replace("1分の",""),"円"]);
            }
        }
        else{
            string[] TSF_calcND=.count(TSF_calcA,".")?TSF_calcA.split("."):[TSF_calcA,""];
            string TSF_calcNstr=TSF_calcND[0],TSF_calcDstr=TSF_calcND[$-1];
            TSF_calcNstr=TSF_calc_comma_okusen(TSF_calcNstr,TSF_Calc_okusenyen,4,true).stripLeft('0').replace("円","");
            if( TSF_calcNstr.length==0 ){ TSF_calcNstr="0"; }
            TSF_calcDstr=TSF_calc_comma_rinmou(TSF_calcDstr,TSF_Calc_rinmouyen,1,true).stripLeft('0').replace("円","");
...
//            TSF_calcA=replace(TSF_calcA,TSF_CalcReg_okusen[TSF_okusenK],"+($1)"~TSF_okusenV);
...
//            TSF_calcA=replace(TSF_calcA,TSF_CalcReg_rinmou[TSF_rinmouK],"+($1)"~TSF_rinmouV);
...
auto TSF_Calc_FLRlazy(string TSF_calcQ,string TSF_calcO){    //#三項演算子と「~」を用いてタプルに分割。TSF_calcFは遅延評価。(TSFAPI)
    string TSF_calcF,TSF_calcL,TSF_calcR;
    string[] TSF_calcQsplits=TSF_calcQ.split(TSF_calcO);
...
//         writeln("TSF_calcA,TSF_calcK %s'%s".format(TSF_calcA,TSF_calcK));
    }
    ...
    else if( count(TSF_calcK,"M~") ){
        auto TSF_calcFLR=TSF_Calc_FLRlazy(TSF_calcK,"M~");  string TSF_calcF=TSF_calcFLR[0],TSF_calcL=TSF_calcFLR[1],TSF_calcR=TSF_calcFLR[2];
        long TSF_calcS=TSF_Io_RPNzero(TSF_calcL),TSF_calcG=TSF_Io_RPNzero(TSF_calcR);
        if( TSF_calcS<=TSF_calcG ){ TSF_calcG++; }else{ TSF_calcS=TSF_calcG; TSF_calcG=TSF_calcS+1; }
        string TSF_calcM="0";
        if( count(TSF_calcF,":") ){
            TSF_calcA="n|0";
        }
        else{
            if( count(TSF_calcF,"k") ){
                foreach(long TSF_calcC;TSF_calcS..TSF_calcG){
                    TSF_calcM=join([TSF_calcM,TSF_Calc_addition(TSF_calcF.replace("k",to!string(TSF_calcC)))],"+");
                }
            }
            else{
                TSF_calcF=TSF_Calc_addition(TSF_calcF);  if( count(TSF_calcF,"|")==0 ){ TSF_calcF=TSF_Calc_addition(TSF_calcF); }
                TSF_calcM=join([TSF_Calc_addition(TSF_calcF),to!string(TSF_calcG-TSF_calcS)],"*");
            }
...
//        TSF_calcA=TSF_Calc_addition(TSF_calcM);
    ...
    else if( count(TSF_calcK,"P~") ){
        auto TSF_calcFLR=TSF_Calc_FLRlazy(TSF_calcK,"P~");  string TSF_calcF=TSF_calcFLR[0],TSF_calcL=TSF_calcFLR[1],TSF_calcR=TSF_calcFLR[2];
        long TSF_calcS=TSF_Io_RPNzero(TSF_calcL),TSF_calcG=TSF_Io_RPNzero(TSF_calcR);
        if( TSF_calcS<=TSF_calcG ){ TSF_calcG++; }else{ TSF_calcS=TSF_calcG; TSF_calcG=TSF_calcS+1; }
        string TSF_calcP="1";
        if( count(TSF_calcF,":") ){
            ...
            if( count(TSF_calcF,"k") ){
                foreach(long TSF_calcC;TSF_calcS..TSF_calcG){
                    TSF_calcP=join([TSF_calcP,TSF_Calc_addition(TSF_calcF.replace("k",to!string(TSF_calcC)))],"*");
                }
            }
            else{
//                TSF_calcF=TSF_Calc_addition(TSF_calcF);  if( count(TSF_calcF,"|")==0 ){ TSF_calcF=TSF_Calc_addition(TSF_calcF); }
//                TSF_calcL=TSF_calcF.split("|")[0];  TSF_calcR=TSF_calcF.split("|")[$-1];
//                TSF_calcP=join([to!string(pow(to!long(TSF_calcL),TSF_calcG-TSF_calcS)),to!string(pow(to!long(TSF_calcR),TSF_calcG-TSF_calcS))],"|");
                foreach(long TSF_calcC;TSF_calcS..TSF_calcG){
                    TSF_calcP=join([TSF_calcP,TSF_Calc_addition(TSF_calcF.replace("k",to!string(TSF_calcC)))],"*");
                }
            }
        ...
        if( BigInt(TSF_calcRD)==0 ){
            ...
            switch( TSF_calcO ){
                ...
                case '#':
                    ...
                    if( TSF_calcRM>0 ){
                        if( TSF_calcLN>=0 ){
                            TSF_calcLN=abs(TSF_calcLN)%TSF_calcRM;
                        }
                        else{
                            TSF_calcLN=-abs(TSF_calcLN)%TSF_calcRM;
                        }
                        ...
                        if( TSF_calcLN>=0 ){
                            TSF_calcLN=abs(TSF_calcLN)%abs(TSF_calcRM);
                        }
                        else{
                            TSF_calcLN=abs(TSF_calcRM)-abs(TSF_calcLN)%abs(TSF_calcRM);
                        }
                    }
                    else{
                        TSF_calcA="n|0";
...
//    writeln("TSF_calcN,TSF_calcD,TSF_calcM %s,%s,%s".format(TSF_calcN,TSF_calcD,TSF_calcM));
    if( TSF_calcD!="" ){
